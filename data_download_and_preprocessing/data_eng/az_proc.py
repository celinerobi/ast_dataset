...
def lons_lat_to_filepaths(lons, lats, index):
    ...
    for i in tqdm(range(len(lons))):
        ...
        if naip_file_pathways != None:
            ...
            select_path = select_path[select_path[:,
                                      2] >= "2018"]  # filter out years to get the most recent data that will include the highest resolution data

            select_path = select_path[
                (select_path[:, 6] == "60cm") | (select_path[:, 6] == "060cm")]  # select only pathways with 60cm
...
class annotator:
    ...
    def track_tile_annotations(self, tile_name_tile_url_labeled):
        ...
        self.tile_name_tile_url_labeled = np.load(
            tile_name_tile_url_labeled)  # the tiles that have not yet been labeled to date
        self.tile_name_tile_url_labeled = np.concatenate(
            (self.tile_name_tile_url_labeled, self.tile_name_tile_url_tiles_for_annotators), axis=0)
        print("Labeled Tiles", self.tile_name_tile_url_labeled.shape)

        self.tile_name_tile_url_remaining = np.delete(self.tile_name_tile_url_unlabeled, range(self.num_tiles),
                                                      0)  # the numpy array of the remaining tiles
    ...
    def move_images_annotations_to_complete_dataset(self, complete_dir, include_tiles=False, original=False,
        ...
        Args:
            ...
            xml_folder_name(str): name of folder in complete dataset directory that contains annotations
        Returns:
            len(annotations): number of annotations
            len(images): number of images
        """
        # make a complete dataset
        self.complete_dataset_xml_dir = os.path.join(complete_dir, "complete_dataset", xml_folder_name)
...
def verification_folders_specify_in_function(home_directory, set_number):
    ...
    for i in range(len(folder_names)):
        ...
        # create verification subfolder for each group
        os.makedirs(os.path.join(verification_dir, "verify_" + folder_names[i] + "_" + set_number),
                    exist_ok=True)  # verification folder for each group
        os.makedirs(os.path.join(verification_dir, "verify_" + folder_names[i] + "_" + set_number, "chips"),
                    exist_ok=True)  # image sub folder
        os.makedirs(os.path.join(verification_dir, "verify_" + folder_names[i] + "_" + set_number, "chips_xml"),
                    exist_ok=True)  # xml sub folder
...
def seperate_images_for_verification_update_tracking_specify_in_function(folder_annotator_list, verification_dir,
        ...
        for ii in range(len(folder_annotator_list[i][1])):  # iterate over annotator
            ...
            for iii in np.where(tile_img_annotation_annotator[:, 4] == folder_annotator_list[i][1][ii])[0]:
                ...
                if len(tile_img_annotation_annotator[iii, 5]) == 0:
                    tile_img_annotation_annotator[iii, 5] = folder_annotator_list[i][0].split("_")[
                        0].capitalize()  # coverage
                    tile_img_annotation_annotator[iii, 6] = folder_annotator_list[i][0].split("_")[
                        1].capitalize()  # quality
                    tile_img_annotation_annotator[iii, 7] = folder_annotator_list[i][0].split("_")[
                        2].capitalize()  # class

                    shutil.copy(tile_img_annotation_annotator[iii, 2],
                                os.path.join(verification_dir,
                                             "verify_" + folder_annotator_list[i][0] + "_" + set_number,
                                             "chips"))  # copy images

                    shutil.copy(
                        os.path.join(tile_img_annotation_annotator[iii, 2].rsplit("/", 1)[0], "chips_positive_xml",
                                     tile_img_annotation_annotator[iii, 3]),
                        os.path.join(verification_dir, "verify_" + folder_annotator_list[i][0] + "_" + set_number,
                                     "chips_xml"))  # copy annotations
...
def dataset_summary_assessment(img_annotation_path, multiple=True):
    ...
    if multiple == True:
        for i in range(len(img_annotation_path)):
            summary_table_temp, unknown_object_array_temp, number_of_images_temp = summary_of_dataset(
                img_annotation_path[i, 0],
